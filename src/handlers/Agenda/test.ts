import { Input, getConflict, processInputs } from ".";
...
const conflicts = getConflict(testInputs);

console.log("Conflicts:");
conflicts.forEach(([input1, input2], index) => {
  console.log(`Conflict ${index + 1}:`);
  console.log(
    `  - ${input1.name} (${input1.type}): ${input1.startTime} for ${input1.duration}`
  );
  console.log(
    `  - ${input2.name} (${input2.type}): ${input2.startTime} for ${input2.duration}`
  );
});


const testInputs2: Input[] = [
  {
...
const processedInputs = processInputs(testInputs2);
